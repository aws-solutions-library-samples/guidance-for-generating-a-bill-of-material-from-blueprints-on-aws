...
export class BackendStack extends cdk.NestedStack {
  ...
  constructor(scope: Construct, id: string, props: BackendStackProps) {
    ...
    // Blueprint Analyzer resources: S3 bucket + DynamoDB jobs table
    if (props.config.backend?.pattern === "blueprint-analyzer") {
      this.createBlueprintResources(props.config)
    }
  }
  ...
  private createAgentCoreRuntime(config: AppConfig): void {
    ...
    if (pattern === "blueprint-analyzer") {
      envVars["BLUEPRINT_BUCKET"] = `${config.stack_name_base.toLowerCase()}-blueprints-${this.account}`
      envVars["BLUEPRINT_TABLE"] = `${config.stack_name_base}-jobs`
    }
  ...
  private createBlueprintResources(config: AppConfig): void {
    // S3 bucket for PDF uploads and analysis outputs
    const blueprintBucket = new s3.Bucket(this, "BlueprintBucket", {
      bucketName: `${config.stack_name_base.toLowerCase()}-blueprints-${this.account}`,
    ...
    const jobsTable = new dynamodb.Table(this, "BlueprintJobsTable", {
    ...
    // Explicitly set CORS via SDK call (more reliable than CFN bucket property updates)
    new cr.AwsCustomResource(this, "BlueprintBucketCors", {
      onCreate: {
        service: "S3",
        action: "putBucketCors",
        parameters: {
          Bucket: blueprintBucket.bucketName,
          CORSConfiguration: {
            CORSRules: [
              {
                AllowedHeaders: ["*"],
        ...
        physicalResourceId: cr.PhysicalResourceId.of("BlueprintBucketCorsConfig"),
      },
      onUpdate: {
        service: "S3",
        action: "putBucketCors",
      ...
      policy: cr.AwsCustomResourcePolicy.fromStatements([
        new iam.PolicyStatement({
          ...
          resources: [blueprintBucket.bucketArn],
        }),
      ]),
    })

    // Grant the agent runtime role access via the AgentCoreRole created earlier in this stack
    const agentRole = this.node.tryFindChild("AgentCoreRole") as iam.IRole
    if (agentRole) {
      blueprintBucket.grantReadWrite(agentRole)
      jobsTable.grantReadWriteData(agentRole)
    }

    // Store resource names in SSM for the runtime to pick up
    new ssm.StringParameter(this, "BlueprintBucketParam", {
      parameterName: `/${config.stack_name_base}/blueprint-bucket`,
      stringValue: blueprintBucket.bucketName,
    })

    new ssm.StringParameter(this, "BlueprintTableParam", {
      parameterName: `/${config.stack_name_base}/blueprint-table`,
      stringValue: jobsTable.tableName,
    })

    // Blueprint API Lambda for uploads and job queries
    const blueprintApiLambda = new PythonFunction(this, "BlueprintApiLambda", {
      functionName: `${config.stack_name_base}-blueprint-api`,
      runtime: lambda.Runtime.PYTHON_3_13,
      architecture: lambda.Architecture.ARM_64,
      entry: path.join(__dirname, "..", "lambdas", "blueprint-api"),
      handler: "handler",
      environment: {
        BLUEPRINT_BUCKET: blueprintBucket.bucketName,
        BLUEPRINT_TABLE: jobsTable.tableName,
      },
      memorySize: 256,
      timeout: cdk.Duration.seconds(60),
      logGroup: new logs.LogGroup(this, "BlueprintApiLogGroup", {
        logGroupName: `/aws/lambda/${config.stack_name_base}-blueprint-api`,
        retention: logs.RetentionDays.ONE_WEEK,
        removalPolicy: cdk.RemovalPolicy.DESTROY,
      }),
    })

    blueprintBucket.grantReadWrite(blueprintApiLambda)
    blueprintApiLambda.addToRolePolicy(
      new iam.PolicyStatement({
        actions: ["s3:PutBucketCORS", "s3:GetBucketCORS"],
        resources: [blueprintBucket.bucketArn],
      })
    )
    jobsTable.grantReadWriteData(blueprintApiLambda)

    // API Gateway for blueprint endpoints
    const blueprintApi = new apigateway.RestApi(this, "BlueprintApi", {
      restApiName: `${config.stack_name_base}-blueprint-api`,
      description: "API for blueprint upload and job management",
      binaryMediaTypes: [],
      defaultCorsPreflightOptions: {
    ...
    const blueprintAuthorizer = new apigateway.CognitoUserPoolsAuthorizer(
      this,
      "BlueprintApiAuthorizer",
      {
        cognitoUserPools: [this.userPool],
        identitySource: "method.request.header.Authorization",
      }
    ...
    const blueprintResource = blueprintApi.root.addResource("blueprint")
    const uploadResource = blueprintResource.addResource("upload")
    const jobsResource = blueprintResource.addResource("jobs")
    const jobByIdResource = jobsResource.addResource("{jobId}")
    const resultsResource = blueprintResource.addResource("results")
    const resultByIdResource = resultsResource.addResource("{jobId}")
    const resultPagesResource = resultByIdResource.addResource("pages")

    const lambdaIntegration = new apigateway.LambdaIntegration(blueprintApiLambda)

    uploadResource.addMethod("POST", lambdaIntegration, {
      authorizer: blueprintAuthorizer,
      authorizationType: apigateway.AuthorizationType.COGNITO,
    })
    ...
    jobByIdResource.addMethod("GET", lambdaIntegration, {
      authorizer: blueprintAuthorizer,
      authorizationType: apigateway.AuthorizationType.COGNITO,
    })

    jobByIdResource.addMethod("DELETE", lambdaIntegration, {
      authorizer: blueprintAuthorizer,
      authorizationType: apigateway.AuthorizationType.COGNITO,
    })

    resultByIdResource.addMethod("GET", lambdaIntegration, {
      authorizer: blueprintAuthorizer,
      authorizationType: apigateway.AuthorizationType.COGNITO,
    })

    resultPagesResource.addMethod("GET", lambdaIntegration, {
      authorizer: blueprintAuthorizer,
      authorizationType: apigateway.AuthorizationType.COGNITO,
    })

    // Store API URL in SSM and expose for frontend
    this.blueprintApiUrl = blueprintApi.url

    new ssm.StringParameter(this, "BlueprintApiUrlParam", {
      parameterName: `/${config.stack_name_base}/blueprint-api-url`,
      stringValue: blueprintApi.url,
    })

    new cdk.CfnOutput(this, "BlueprintApiUrl", {
      value: blueprintApi.url,
      description: "Blueprint API Gateway URL",
      exportName: `${config.stack_name_base}-BlueprintApiUrl`,
    })

    new cdk.CfnOutput(this, "BlueprintBucketName", {
      value: blueprintBucket.bucketName,
      description: "S3 bucket for blueprint PDFs and outputs",
      exportName: `${config.stack_name_base}-BlueprintBucket`,
    })

    new cdk.CfnOutput(this, "BlueprintJobsTableName", {
      value: jobsTable.tableName,
      description: "DynamoDB table for analysis job tracking",
      exportName: `${config.stack_name_base}-JobsTable`,
    })
  }
