...
The pipeline processes uploaded PDF blueprints through the following steps:
...
| Amazon CloudFront | Included with Amplify | $ 0.00 |
| **Total** | | **$ 77.37** |
...
For automated deployment, a one-click deploy script (`scripts/deploy.sh`) is available. This script automates all deployment steps including prerequisite checks, dependency installation, CDK bootstrap, resource creation, and validation.

**Usage:**

```bash
# Clone the repository
git clone https://github.com/aws-solutions-library-samples/guidance-for-generating-a-bill-of-materials-from-blueprints-on-aws.git
cd guidance-for-generating-a-bill-of-materials-from-blueprints-on-aws
...
- Verifies all prerequisites are installed (AWS CLI, Node.js, Python, Docker, CDK)
- Validates AWS credentials are configured
- Prompts for target AWS Region (defaults to us-east-1)
- Checks and runs CDK bootstrap if needed
- Installs CDK project dependencies
- Deploys the full backend stack (Cognito, AgentCore Runtime, API Gateway, S3, DynamoDB)
- Builds and deploys the React frontend to AWS Amplify
- Validates the deployment and prints the application URL
...
Follow these steps to deploy the Guidance manually:
...
- **Custom output formats:** Modify the structured output tools in `patterns/blueprint-analyzer/blueprint_analyzer/tools/structured_tools.py` to produce additional formats (CSV, Excel, IFC).
- **Batch processing:** Add a queue-based workflow using Amazon SQS to process multiple blueprints concurrently.
- **Fine-tune prompts:** Adjust agent prompts in `patterns/blueprint-analyzer/blueprint_analyzer/agents.py` for specific blueprint types (mechanical, electrical, plumbing).
- **Add authentication providers:** Replace or augment Cognito with your existing identity provider using the OIDC configuration in `frontend/src/hooks/useAuth.ts`.
...
For any feedback, questions, or suggestions, please use the [Issues tab](https://github.com/aws-solutions-library-samples/guidance-for-generating-a-bill-of-materials-from-blueprints-on-aws/issues) in this repository.
